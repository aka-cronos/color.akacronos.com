 ...
 */
export function exportCss(name: string, steps: PaletteStep[], darkSteps: PaletteStep[], neutralSteps?: PaletteStep[]): string {
  ...
  const neutralVars = neutralSteps
    ? "\n" + neutralSteps.map((s) => `  --color-neutral-${s.name}: ${s.cssOklch};`).join("\n")
    : "";
 ...
 */
export function exportTailwind(name: string, steps: PaletteStep[], neutralSteps?: PaletteStep[]): string {
  const vars = steps
    .map((s) => `  --color-${name}-${s.name}: ${s.cssOklch};`)
    .join("\n");
  const neutralVars = neutralSteps
 ...
 */
export function exportJson(name: string, steps: PaletteStep[], neutralSteps?: PaletteStep[]): string {
